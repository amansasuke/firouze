<?php get_header(); ?>

<!-- Hero Section -->
<section class="hero">
  <div class="container">
    <div class="hero-content">
      <p class="hero-eyebrow">A Persian Legacy</p>
      <h1>From the Gem of Neyshabur to Every Cup</h1>
      <p class="hero-text">Firouze Tea is inspired by the timeless turquoise stone of Neyshabur and crafted from the lush tea gardens of Lahijan, carrying centuries of Persian tradition into the world.</p>
      <div class="hero-actions">
        <a href="#wholesale" class="btn btn-gold">Become a Retail Partner</a>
        <a href="#collection" class="btn btn-outline">Explore Collection</a>
      </div>
    </div>
  </div>
</section>

<!-- Our Story Section -->
<section class="story-section" id="story">
  <img class="story-pendant" src="<?php echo esc_url( get_template_directory_uri() . '/assets/pendant.png' ); ?>" alt="">
  <div class="container">
    <p class="section-label">Our Story</p>
    <h2 class="section-title">A Heritage Carved in Time</h2>

    <div class="timeline">
      <div class="tl-item">
        <div class="tl-icon"><span>&#128142;</span></div>
        <span class="tl-dot"></span>
        <h3>The Gem of Neyshabur</h3>
        <p>A symbol of protection, luck &amp; calm.</p>
      </div>
      <div class="tl-item">
        <div class="tl-icon"><span>&#127963;</span></div>
        <span class="tl-dot"></span>
        <h3>Ancient Persia</h3>
        <p>Treasured by poets, kings &amp; travelers.</p>
      </div>
      <div class="tl-item">
        <div class="tl-icon"><span>&#9968;</span></div>
        <span class="tl-dot"></span>
        <h3>Neyshabur Mines</h3>
        <p>Where the legendary Firouze stone was born.</p>
      </div>
      <div class="tl-item">
        <div class="tl-icon"><span>&#128042;</span></div>
        <span class="tl-dot"></span>
        <h3>Persian Trade Routes</h3>
        <p>Shared across lands, a sign of heritage.</p>
      </div>
      <div class="tl-item">
        <div class="tl-icon"><span>&#127811;</span></div>
        <span class="tl-dot"></span>
        <h3>Lahijan Tea Gardens</h3>
        <p>Where mist meets the hills and tea thrives.</p>
      </div>
      <div class="tl-item">
        <div class="tl-icon"><span>&#129689;</span></div>
        <span class="tl-dot"></span>
        <h3>Firouze Tea</h3>
        <p>Crafted with passion. Shared with the world.</p>
      </div>
    </div>

    <div class="journey-pill">
      <span>See Our Journey</span>
      <a href="#" aria-label="Facebook" class="social social-fb">
        <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><path d="M13.5 21v-8h2.7l.4-3h-3.1V8.1c0-.9.3-1.5 1.6-1.5h1.6V3.9c-.3 0-1.2-.1-2.3-.1-2.3 0-3.9 1.4-3.9 4v2.2H7.9v3h2.6v8h3z"/></svg>
      </a>
      <a href="#" aria-label="Instagram" class="social social-ig">
        <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><path d="M12 2.2c3.2 0 3.6 0 4.9.1 1.2.1 1.8.2 2.2.4.6.2 1 .5 1.4.9.4.4.7.8.9 1.4.2.4.4 1 .4 2.2.1 1.3.1 1.7.1 4.9s0 3.6-.1 4.9c-.1 1.2-.2 1.8-.4 2.2-.2.6-.5 1-.9 1.4-.4.4-.8.7-1.4.9-.4.2-1 .4-2.2.4-1.3.1-1.7.1-4.9.1s-3.6 0-4.9-.1c-1.2-.1-1.8-.2-2.2-.4-.6-.2-1-.5-1.4-.9-.4-.4-.7-.8-.9-1.4-.2-.4-.4-1-.4-2.2-.1-1.3-.1-1.7-.1-4.9s0-3.6.1-4.9c.1-1.2.2-1.8.4-2.2.2-.6.5-1 .9-1.4.4-.4.8-.7 1.4-.9.4-.2 1-.4 2.2-.4 1.3-.1 1.7-.1 4.9-.1zm0 1.8c-3.1 0-3.5 0-4.8.1-1.1.1-1.4.2-1.7.3-.4.2-.7.4-1 .7-.3.3-.5.6-.7 1-.1.3-.3.6-.3 1.7-.1 1.3-.1 1.7-.1 4.8s0 3.5.1 4.8c.1 1.1.2 1.4.3 1.7.2.4.4.7.7 1 .3.3.6.5 1 .7.3.1.6.3 1.7.3 1.3.1 1.7.1 4.8.1s3.5 0 4.8-.1c1.1-.1 1.4-.2 1.7-.3.4-.2.7-.4 1-.7.3-.3.5-.6.7-1 .1-.3.3-.6.3-1.7.1-1.3.1-1.7.1-4.8s0-3.5-.1-4.8c-.1-1.1-.2-1.4-.3-1.7-.2-.4-.4-.7-.7-1-.3-.3-.6-.5-1-.7-.3-.1-.6-.3-1.7-.3-1.3-.1-1.7-.1-4.8-.1zm0 3.1a4.9 4.9 0 1 1 0 9.8 4.9 4.9 0 0 1 0-9.8zm0 8.1a3.2 3.2 0 1 0 0-6.4 3.2 3.2 0 0 0 0 6.4zm6.2-8.3a1.1 1.1 0 1 1-2.3 0 1.1 1.1 0 0 1 2.3 0z"/></svg>
      </a>
      <a href="#" aria-label="X" class="social social-x">
        <svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor"><path d="M18.9 2H22l-6.8 7.8L23.2 22h-6.3l-4.9-6.4L6.4 22H3.3l7.3-8.3L1.6 2H8l4.4 5.9L18.9 2zm-1.1 18.1h1.7L7.1 3.8H5.3l12.5 16.3z"/></svg>
      </a>
    </div>
  </div>
</section>

<!-- Collection Section -->
<section class="collection-section" id="collection">
  <div class="container-wide">
    <p class="section-label gold"><span>Collection</span></p>
    <h2 class="section-title light">The Expressions of Persia</h2>
    <div class="gold-divider"></div>

    <div class="tea-cards">
      <a class="tea-card tea-cinnamon" href="<?php echo esc_url( home_url( '/cinnamon-tea/' ) ); ?>">
        <img class="tea-jar" src="<?php echo esc_url( get_template_directory_uri() . '/assets/jar-cinnamon.png' ); ?>" alt="Cinnamon Tea jar">
        <h3>Cinnamon Tea</h3>
        <p>Comforting and naturally spiced premium Persian cinnamon for a warm, soothing infusion.</p>
      </a>
      <a class="tea-card tea-black" href="<?php echo esc_url( home_url( '/black-tea/' ) ); ?>">
        <img class="tea-jar" src="<?php echo esc_url( get_template_directory_uri() . '/assets/jar-cinnamon.png' ); ?>" alt="Black Tea jar">
        <h3>Black Tea</h3>
        <p>Bold, rich and aromatic black tea crafted from premium Persian leaves.</p>
      </a>
      <a class="tea-card tea-cardamom" href="<?php echo esc_url( home_url( '/cardamom-tea/' ) ); ?>">
        <img class="tea-jar" src="<?php echo esc_url( get_template_directory_uri() . '/assets/jar-cinnamon.png' ); ?>" alt="Cardamom Tea jar">
        <h3>Cardamom Tea</h3>
        <p>Cool, aromatic and refreshing, blended with premium Persian cardamom for a smooth, uplifting cup.</p>
      </a>
      <a class="tea-card tea-saffron" href="<?php echo esc_url( home_url( '/saffron-tea/' ) ); ?>">
        <img class="tea-jar" src="<?php echo esc_url( get_template_directory_uri() . '/assets/jar-cinnamon.png' ); ?>" alt="Saffron Tea jar">
        <h3>Saffron Tea</h3>
        <p>Golden, fragrant and delicately infused with premium Persian saffron for a luxurious tea experience.</p>
      </a>
      <a class="tea-card tea-green" href="<?php echo esc_url( home_url( '/green-tea/' ) ); ?>">
        <img class="tea-jar" src="<?php echo esc_url( get_template_directory_uri() . '/assets/jar-cinnamon.png' ); ?>" alt="Green Tea jar">
        <h3>Green Tea</h3>
        <p>Fresh, light and revitalizing tea crafted from carefully selected Persian tea leaves.</p>
      </a>
    </div>

    <div class="gold-divider"></div>
  </div>
</section>

<!-- Crafted with Care Section -->
<section class="craft-section" id="craft">
  <div class="craft-panel">
    <div class="container-wide craft-inner">
      <div class="craft-intro">
        <p class="craft-label">Crafted with Care</p>
        <h2>The Art of Persian Tea</h2>
        <p class="craft-text">Every leaf follows a meticulous journey from garden to jar, ensuring exceptional quality, aroma and taste in every cup.</p>
      </div>
      <div class="craft-steps">
        <div class="craft-step">
          <span class="craft-icon">&#127811;</span>
          <p>Handpicked<br>Leaves</p>
        </div>
        <div class="craft-step">
          <span class="craft-icon">&#9749;</span>
          <p>Careful<br>Selection</p>
        </div>
        <div class="craft-step">
          <span class="craft-icon">&#9832;</span>
          <p>Traditional<br>Drying</p>
        </div>
        <div class="craft-step">
          <span class="craft-icon">&#129750;</span>
          <p>Expert<br>Blending</p>
        </div>
        <div class="craft-step">
          <span class="craft-icon">&#129689;</span>
          <p>Premium<br>Packaging</p>
        </div>
        <div class="craft-step">
          <span class="craft-icon">&#127760;</span>
          <p>Global<br>Distribution</p>
        </div>
      </div>
    </div>
  </div>
</section>

<!-- Partner with Us / Wholesale Section -->
<section class="partner-section" id="wholesale">
  <div class="container">
    <p class="section-label gold">Partner with Us</p>
    <p class="partner-text">We work with retailers, supermarkets, importers, hotels, restaurants, caf&eacute;s and distributors worldwide.</p>
    <div class="gold-divider"></div>

    <div class="partner-grid">
      <div class="partner-item">
        <span class="partner-icon">&#127978;</span>
        <p>Retailers</p>
      </div>
      <div class="partner-item">
        <span class="partner-icon">&#128674;</span>
        <p>Importers</p>
      </div>
      <div class="partner-item">
        <span class="partner-icon">&#128722;</span>
        <p>Supermarkets</p>
      </div>
      <div class="partner-item">
        <span class="partner-icon">&#127963;</span>
        <p>Hotels</p>
      </div>
      <div class="partner-item">
        <span class="partner-icon">&#9749;</span>
        <p>Cafes</p>
      </div>
      <div class="partner-item">
        <span class="partner-icon">&#128666;</span>
        <p>Distributors</p>
      </div>
    </div>

    <div class="partner-cta">
      <a href="#contact" class="btn btn-gold">Request Wholesale Catalogue</a>
    </div>
  </div>
</section>

<?php get_footer(); ?>
